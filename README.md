...
使用 `jlink/daplink/...` 等等 swd 调试器进行固件烧录，可使用 keil 或其他 swd 烧录工具，固件在 pcb 开源页面附件有提供

## 六、调试

如果您需要对本项目进行二次开发或拓展，那么应该需要一些调试手段。

本项目没有引出串口，输入输出信息是通过 `segger RTT` 保存在一块 ram 中的，当调试器链接后，电脑用调试器连接后通过 `openOCD` 来对内存进行写入读出以实现输入输出，理论上，这会比串口更快，毕竟只要读写内存，而无需外设收发，并且还能保存一定的历史输出。  
